...
init_error_logger("ai-test-case-generator-main")# Initialize error logging before any other imports
from utils.error_logger import init_error_logger, capture_exception, capture_message, set_tag, set_context
    ...
    try:
        ...
        with generation_status['lock']:
            ...
            try:
                ...
                # Decide async vs sync execution based on env (Render: set RUN_ASYNC_URL=false)
                RUN_ASYNC = os.getenv("RUN_ASYNC_URL", "true").lower() == "true"
                USE_SELENIUM = os.getenv("USE_SELENIUM", "false").lower() == "true"

                if not RUN_ASYNC:
                    # Synchronous URL generation for production reliability
                    try:
                        with generation_status['lock']:
                            generation_status['phase'] = 'ai_generation'
                            generation_status['log'].append(f"[SYNC] Generating test cases from URL content for types: {test_case_types}")

                        generation_start_time = datetime.utcnow()

                        test_cases_local = generate_url_test_cases(url, test_case_types, use_selenium=USE_SELENIUM)
                        logger.info(f"[URL SYNC] Direct URL generation finished, has content: {bool(test_cases_local)}")
                        ...
                        test_cases_filename_local = f"url_test_cases_{url_key}.txt"
                        ...
                        with open(test_cases_filepath_local, 'w', encoding='utf-8') as f:
                            f.write(f"URL: {url}\n")
                            f.write("Generated Test Cases (via direct URL content analysis):\n\n")
                            f.write(test_cases_local)

                        # Generate Excel file
                        file_base_name = f"url_test_cases_{url_key}"
                        excel_file = save_excel_report(test_cases_local, file_base_name)
                        logger.info(f"[URL SYNC] Generated Excel file: {excel_file}")

                        # Parse to structured format
                        structured_test_data = parse_traditional_format(test_cases_local)
                        if isinstance(structured_test_data, str) or not isinstance(structured_test_data, list):
                        ...
                        url_key_final = mongo_handler_local.save_test_case({
                            ...
                            'test_data': structured_test_data
                        }, url_key, 'url', current_user.get('id') if current_user else None)
                        logger.info(f"[URL SYNC] Saved test case with URL key: {url_key_final}")

                        # Track analytics
                        generation_end_time = datetime.utcnow()
                        generation_duration = (generation_end_time - generation_start_time).total_seconds()
                        mongo_handler_local.track_event({
                        ...
                        with generation_status['lock']:
                            ...
                            generation_status['log'].append('Generation completed (sync)')
                            generation_status['final_url_key'] = url_key_final
                            logger.info(f"[URL SYNC] Set final_url_key in generation status: {url_key_final}")

                        # Return immediately so frontend can poll and redirect using final_url_key
                        ...
                        with generation_status['lock']:
                            ...
                            generation_status['log'].append(f"Error: {e}")
                        return jsonify({'error': f'Error processing URL content: {str(e)}'}), 500

                # Async path (kept for local/dev)
                import threading
                ...
                def _run_url_generation_async(target_url, types, result_key, user_id=None):
                    try:
                        ...
                        # 1) Fetch website content directly -> 2) Generate test cases
                        ...
                        test_cases_local = generate_url_test_cases(target_url, types, use_selenium=USE_SELENIUM)
                ...
                threading.Thread(
                    target=_run_url_generation_async,
                    args=(url, test_case_types, url_key, current_user.get('id') if current_user else None),
                    daemon=True
                ).start()
